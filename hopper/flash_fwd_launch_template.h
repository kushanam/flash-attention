...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    //TODO: remove `Arch < 100` once blackwell native FAv3 is inetgrated
    static constexpr isHopper = Arch >= 90 && Arch < 100;
    ...
    using ArchTag = std::conditional_t<isHopper, cutlass::arch::Sm90, cutlass::arch::Sm80>;
    ...
    static constexpr int kBlockM = isHopper ? std::get<0>(kBlockMN_RS_IntraWGOverlap) : std::get<0>(kBlockMN_kNWarps_Stages_RS);
    static constexpr int kBlockN = isHopper ? std::get<1>(kBlockMN_RS_IntraWGOverlap) : std::get<1>(kBlockMN_kNWarps_Stages_RS);
    ...
    static constexpr int kStages = isHopper ? 2 : std::get<3>(kBlockMN_kNWarps_Stages_RS);
    static constexpr bool Q_in_regs = isHopper ? false : std::get<4>(kBlockMN_kNWarps_Stages_RS);
    ...
    static constexpr int NumProducerThreads = isHopper ? CollectiveMainloop::NumProducerThreads : CollectiveMainloop::NumMmaThreads;
    using SchedulerPersistent = std::conditional_t<Varlen,
        flash::VarlenDynamicPersistentTileScheduler<kBlockM, CollectiveMainloop::NumMmaThreads, NumProducerThreads, Split, PackGQA, isHopper /*WarpSpecialized*/>,
        std::conditional_t<!Is_causal && !Is_local,
            flash::StaticPersistentTileScheduler<Split>,
            flash::DynamicPersistentTileScheduler<CollectiveMainloop::NumMmaThreads, NumProducerThreads, Split, PackGQA, isHopper /*WarpSpecialized*/>
        >
    >;
    ...
    using Scheduler = std::conditional_t<isHopper ? (Split && !Varlen) : !((Is_causal && !Varlen) || (Varlen && Split)), SchedulerSingleTile, SchedulerPersistent>;
    ...
    CAUSAL_LOCAL_SWITCH(params.is_causal, params.is_local, Is_causal, Is_local, [&] {
        VCOLMAJOR_SWITCH(params.v_dim_stride != 1, V_colmajor_, [&] {
            ...
            VARLEN_SWITCH(params.cu_seqlens_q || params.cu_seqlens_k || params.seqused_q || params.seqused_k || params.leftpad_k, Varlen, [&] {
                ...
                static constexpr int kBlockM = isHopper ? std::get<0>(tile_size_fwd_sm90(kHeadDim, kHeadDimV, Is_causal, Is_local, sizeof(T) /*element_size*/, V_colmajor, PagedKV, Has_softcap)) : 128;

                // On nvcc 12.8, hdim 128, without cluster is faster (730 vs 700 TFLOPS)
                static constexpr bool Enable_cluster = isHopper && (sizeof(T) == 2 ? (kHeadDim >= 192) : (kHeadDim == 192)) && !Is_causal && !Is_local && !Split && !PagedKV && !Varlen;
                BOOL_SWITCH(params.qv_ptr, HasQV_, [&] {
                    static constexpr bool HasQv = HasQV_ && isHopper && !Is_FP8 && kHeadDim == 64 && kHeadDimV == 512;
